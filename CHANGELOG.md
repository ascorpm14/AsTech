...
## Run #35 — 26/06/2026
- **Amélioration :** Suppression du scroll-indicator sur mobile (≤768px) + resserrement du padding hero. Le scroll-indicator (« Weiter » avec chevrons animés) était un élément desktop-only qui créait un « no man's land » de ~72px entre la hero section et les features sur mobile. Sur écran tactile, les utilisateurs scrollent naturellement — aucun indicateur visuel n'est nécessaire. La suppression de cet espace mort, combinée à la réduction du padding-bottom du hero de 80px à 40px (et padding-latéral de 24px à 16px), resserre le flow vertical : les onglets de scénario (juste sous le mockup téléphone) mènent directement à la section Features sans zone de transition vide. Le padding-top passe de 140px à 120px — toujours suffisant pour compenser la nav fixe (70px). Résultat : ~100px de contenu vertical économisé sur mobile, un flow plus naturel (hero → features sans interruption), et une meilleure utilisation de l'espace limité sur mobile. Pour un Steuerberater allemand consultant le site sur smartphone entre deux rendez-vous, chaque pixel compte — un scroll forcé à travers un espace vide est une micro-friction qui s'accumule.
- **Inspiration :** Apple.com, Linear.app, Vercel — les sites premium adaptent leurs indicateurs de scroll par breakpoint. Sur desktop, le scroll indicator guide l'œil. Sur mobile, il disparaît car le geste tactile de scroll est intuitif. Le principe de « responsive wayfinding » (Nielsen Norman Group) : un élément d'orientation qui n'est utile que sur un seul type d'appareil ne doit pas créer de bruit sur les autres.
- **Section modifiée :** CSS — `.scroll-indicator` : règle `@media (max-width:768px)` passe de `padding-bottom:24px;margin-top:-12px` à `display:none` ; `.hero` : ajout règle `@media (max-width:768px)` avec `padding:120px 16px 40px`
- **Statut :** ✅ Succès
